...
function initWS() {
  ...
  socket.onmessage = function (ws_message) {
    ...
    if (messageType === "chatMessage") {
      ...
      addUser(message);
    } else if (messageType === "dusers") {
      document.getElementById("message_" + message.username).remove();
    } else {
...
function userchatMessageHTML(messageJSON) {
  const username = messageJSON.username;
  const messageId = username;
...
function addUser(messageJSON) {
  const chatMessages = document.getElementById("user-list");
  chatMessages.innerHTML += userchatMessageHTML(messageJSON);
  chatMessages.scrollIntoView(false);
  chatMessages.scrollTop =
    chatMessages.scrollHeight - chatMessages.clientHeight;
}
...
function sendChat() {
  ...
  const xsrf = document.getElementById("xsrf").value;
  ...
  if (ws) {
    ...
    const messageJSON = { message: message, xsrf: xsrf };
